...
**Примечание:** Для работы телеметрии через API необходимо:
1. Запустить API интерпретатор с указанием адреса API сервера (см. [API_TELEMETRY_GUIDE.md](../API_TELEMETRY_GUIDE.md))
2. API интерпретатор будет отправлять телеметрию на API сервер
3. GUI получит телеметрию через GET `/api/telemetry`

Если API интерпретатор запущен без указания адреса API сервера, телеметрия будет недоступна, но команды управления будут работать.

## Телеметрия через API

API сервер поддерживает прием и передачу телеметрии:

1. **Прием телеметрии**: API интерпретатор отправляет телеметрию на API сервер через POST `/api/telemetry`
2. **Хранение**: API сервер хранит последнюю телеметрию в памяти
3. **Получение телеметрии**: Клиенты получают телеметрию через GET `/api/telemetry`

**Важно:** Для работы телеметрии API интерпретатор должен быть запущен с указанием адреса API сервера:
```bash
./crsf_api_interpreter 8082 192.168.1.50 8081
```

Подробнее: [API_TELEMETRY_GUIDE.md](../API_TELEMETRY_GUIDE.md)

## Режим без телеметрии

Основное приложение поддерживает режим работы без телеметрии (для тестирования):
```bash
sudo ./crsf_io_rpi --notel
```

В этом режиме:
- Отключены проверки безопасности, связанные с телеметрией
- API интерпретатор использует неблокирующую запись команд
- Таймауты подключения уменьшены для быстрого ответа
...
- Для телеметрии через API интерпретатор должен быть запущен с указанием адреса API сервера
